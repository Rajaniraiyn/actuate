...
Shared commands select their provider through `--provider` or `ACTUATE_PROVIDER`.
...
```sh
actuate snapshot --format text
actuate --provider apple-simulator --device UUID --device-set /absolute/device/set session
ACTUATE_PROVIDER=apple-simulator ACTUATE_DEVICE=UUID ACTUATE_DEVICE_SET=/absolute/device/set actuate snapshot --format text
actuate apple simulators list
actuate completions zsh
```

`ACTUATE_DEVICE` and `ACTUATE_DEVICE_SET` also have explicit flag overrides.
...
```sh
actuate --provider apple-device discover
actuate --provider apple-device capabilities
actuate --provider apple-device --device DEVICE_UDID discover
actuate --provider apple-device --device DEVICE_UDID capture /absolute/new-frame.bin
```
...
actuate android pair PHONE_IP:PAIRING_PORT --credentials /private/phone
# Or display a QR and discover its matching pairing service.
actuate android pair-qr --credentials /private/phone --timeout-secs 120
...
actuate --provider android --credentials /private/phone --device PHONE_IP:CONNECT_PORT --trust-first-connection discover
actuate --provider android --credentials /private/phone capture /new/screen.png
actuate --provider android --credentials /private/phone session --json
```
