...
target/debug/actuate discover --scope apps
target/debug/actuate observe PID --interactive --limit 80
target/debug/actuate capture /tmp/new-frame.png
target/debug/actuate session --json
```
...
Start it with `cursor_overlay` or by setting `ACTUATE_SOFT_CURSOR=1` before
`actuate session`, which starts the in-process renderer for the whole session.
...
```sh
hyprctl dispatch 'hl.dsp.exec_cmd("ACTUATE_FIXTURE_LOG=/tmp/fixture.log python3 native/linux/fixture.py", { workspace = "3 silent" })'
hyprctl dispatch 'hl.dsp.exec_cmd("GDK_BACKEND=x11 ACTUATE_FIXTURE_ID=dev.actuate.fixture.x11 ACTUATE_FIXTURE_LOG=/tmp/fixture-x11.log python3 native/linux/fixture.py", { workspace = "3 silent" })'
ACTUATE_FIXTURE_LOG=/tmp/fixture.log ACTUATE_FIXTURE_X11_LOG=/tmp/fixture-x11.log python3 tests/linux_e2e.py
```
