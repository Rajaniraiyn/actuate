...
`target/debug/actuate.exe`. For Linux X11, use the logged-in user's DISPLAY and
Xauthority. A Wayland session must not be presented as a complete X11 desktop.

```sh
python3 tests/desktop_smoke.py --binary target/debug/actuate --pid 1234 --capture
```
