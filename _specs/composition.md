...
`actuate` owns portable contracts and `Backend<O, I, C, V, A>`. Platform crates own
...
```rust
use actuate::{Backend, ObservationBudget, ObserveScope, TouchInput};
...
backend.touch(actuate::TouchAction::Tap {
    point: actuate::NormalizedPoint::new(0.5, 0.5)?,
})?;
```
...
`actuate::session::SnapshotHistory` retains the latest 32 observations behind `Arc`,
...
`actuate --provider apple-simulator session` delegates to the library. The optional `actuate-ios` binary
...
conditions throughout implementations. The portable `actuate` and `overlay` contracts
...
`actuate::wait::wait_for_query` accepts any `ObserveScope` provider, a caller-chosen
