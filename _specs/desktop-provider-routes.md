...
| Wayland | Compositor virtual pointer/keyboard, screencopy and image-copy-capture, Hyprland window discovery and shortcuts, layer-shell cursor | Requires the wlr virtual-device protocols; window geometry needs a compositor adapter (Hyprland). Portal RemoteDesktop/libei remains pending. See [Linux](linux.md). |
