...
clang -fobjc-arc -framework AppKit -framework ApplicationServices \
  scripts/macos/drag-probe.m -o /tmp/actuate-drag-probe
/tmp/actuate-drag-probe
```

Read the printed PID and window ID, discover the current window bounds, and use
one persistent Actuate session. Drag the blue rectangle to the green rectangle
