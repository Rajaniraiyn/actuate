...
session available. They exercise the compiled `actuate` and
`actuate-overlay` executables. Python launches fixtures, sends JSON commands,
...
```json
{"op":"cursor_overlay","action":{"kind":"start","executable":"C:/path/to/actuate/target/debug/actuate-overlay.exe","physical_cursor":"hide_while_visible","tracking":"physical_pointer"}}
