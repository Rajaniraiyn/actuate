...
`actuate apple simulators list --json` returns CoreSimulator's native JSON for installed devices, device
...
```sh
actuate --provider apple-simulator --device DEVICE_UUID --device-set /absolute/device/set session --json
```
...
umbrella CLI. The platform-owned `actuate-ios session` binary is available with the
