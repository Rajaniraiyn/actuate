...
if sys.platform == 'darwin':
    r = run('session', ACTUATE_PROVIDER='apple-simulator')
    assert r.returncode and 'requires --device' in r.stderr, r
    r = run('--provider', 'apple-simulator', 'session', ACTUATE_PROVIDER='invalid')
    assert r.returncode and 'requires --device' in r.stderr, r
    r = run('session', '--provider', 'apple-simulator', ACTUATE_DEVICE='test')
    assert r.returncode and 'requires --device-set' in r.stderr, r
else:
    r = run('session', ACTUATE_PROVIDER='apple-simulator')
    assert r.returncode and 'invalid value' in r.stderr, r
for args, env in [(('session',), {'ACTUATE_PROVIDER': 'invalid'}),
...
for shell in ('bash', 'zsh', 'fish'):
    ...
    assert r.returncode == 0 and 'actuate' in r.stdout, r
r = run('spec')
assert r.returncode == 0 and 'ACTUATE_PROVIDER' in r.stdout
