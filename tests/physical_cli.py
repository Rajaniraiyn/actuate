...
def run(*args):
    return subprocess.run(['target/debug/unimation', '--provider', 'apple-device', *args], env=base, text=True, capture_output=True)
...
for args, message in [
    (('capture','/tmp/unimation-should-not-exist.bin'),'requires --device'),
    ...
    (('capture','/tmp/unimation-should-not-exist.bin','--display','1'),'unavailable'),
]:
    r=run(*args)
