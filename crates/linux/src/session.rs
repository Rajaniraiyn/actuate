...
pub enum CaptureSource {
    /// The focused Hyprland monitor, else the first output.
    FocusedOutput {},
    Output {
        name: String,
    },
    ...
    fn execute_core(&mut self, request: SessionRequest) -> Result<Value> {
        match request {
            ...
            SessionRequest::Semantic { target, action } => {
                self.atspi.semantic(&target, action).map(|r| json!(r))
            }
    ...
    fn execute_extension(&mut self, request: LinuxRequest) -> Result<Value> {
        match request {
            ...
            LinuxRequest::HyprlandShortcut {
                ...
                self.hypr()?.dispatch(&lua)?;
                Ok(json!(Receipt::dispatched("linux.hyprland.send_shortcut")))
            }
        }
    }
