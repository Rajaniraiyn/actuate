...
fn failure(message: impl ToString) -> unimation_core::NativeError {
    error("overlay_failed", message, Effect::None)
}
fn serialize(command: Value) -> Result<Vec<u8>> {
    let object = command
        .as_object()
        .ok_or_else(|| failure("Overlay command must be an object"))?;
    let op = object
        .get("op")
        .and_then(Value::as_str)
        .ok_or_else(|| failure("Overlay command requires op"))?;
    let allowed: &[&str] = match op {
        "move" => &["op", "x", "y", "duration_ms"],
        "click" => &["op", "x", "y"],
        "hide" | "show" | "quit" => &["op"],
        _ => return Err(failure("Unknown overlay command")),
    };
    if object.keys().any(|k| !allowed.contains(&k.as_str())) {
        return Err(failure("Unknown overlay command field"));
    }
    if matches!(op, "move" | "click") {
        for key in ["x", "y"] {
            if !object
                .get(key)
            ...
            {
                return Err(failure("Overlay coordinates must be finite numbers"));
            }
        }
    }
    if let Some(duration) = object.get("duration_ms")
        && !duration.as_u64().is_some_and(|v| v <= 10000)
    {
        return Err(failure("duration_ms must be an integer from 0 to 10000"));
    }
...
impl OverlayController {
    ...
    pub fn start(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let writer = match thread::Builder::new()
            .name("unimation-overlay-writer".into())
            .spawn(move || {
    ...
    pub fn stop(&mut self) -> Result<()> {
        ...
        if !stopped {
            ...
            thread::Builder::new()
                .name("unimation-overlay-reaper".into())
                .spawn(move || {
                    let _ = child.wait();
                })
                .map_err(failure)?;
