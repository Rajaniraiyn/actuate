...
Unimation retains the native observation and derives a smaller view for agents. Raw attributes, errors and session-qualified references remain available. Text and compact JSON are presentation formats; neither replaces the stored snapshot.

## Choose an output format

```sh
unimation observe 1234 --scope window --format text --interactive --limit 80
unimation observe 1234 --format compact --limit 80
unimation view snapshot.json --root @e12 --interactive --limit 80
...
These references inform design; they do not establish that a particular Unimation backend or application has passed a live test. See [validation results](validation.md) for tests actually run.
